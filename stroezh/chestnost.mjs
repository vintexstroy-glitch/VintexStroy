...
/**
 * ОБХОД В · ПРОИЗВОДСТВЕНА КОНСТАНТА, СРЕЩУ КОЯТО СЕ СВЕРЯВА ЖИВ ИЗХОД,
 * БЕЗ НИТО ЕДИН ПИН С РЪКА.
 *
 * Обход А лови ЕДНА форма: вход, СМЯТАН от константата (`sled(NAPRED_DNI + 1)`).
 * Но същият дефект има и по-тиха форма — `expect(жив изход).toEqual(KONSTANTA)`.
 * Тя изглежда безупречно: сравнява се произведеното с обявеното. Само че двете
 * страни са ЕДНА и съща — махнеш ли колона от константата, изходът я губи също,
 * и тестът мълчи. Точно това стоеше в `zhurnal-ot-tablitsa`: заглавието му
 * казваше „деветте колони", а деветката не се проверяваше никъде.
 *
 * ПРАВИЛОТО, което го затваря, е ЕДНО: щом жив изход се сверява срещу
 * константа, някъде трябва да стои и ПИН — очакване върху САМАТА константа с
 * ръчно написана стойност (`expect(NAPRED_DNI).toBe(45)`). Пинът е мястото,
 * където числото се твърди; сравнението е мястото, където се ползва.
 *
