...
export async function blok2(ctx: KonteksNaProhoda): Promise<void> {
  ...
  /**
   * ЦЯЛОТО ДЕЙСТВИЕ СЕ ПОВТАРЯ, не само писането · ТРЕТА поправка на това място.
   *
   * Първата сложи чакане и смени само ДУМИТЕ на доклада. Втората обви ПИСАНЕТО
   * в `dokatoStane` — и то държеше полето, но между него и натискането стоят
   * още две стъпки (изборът и самият клик), а прерисуване по всяка от тях пак
   * изяжда написаното. Записът тръгваше ПРАЗЕН и Вратата го отказваше с „Етапът
   * иска име".
   *
   * ХВАНА ГО CI, НЕ МОЯТ ПРОХОД. Локално минаваше — на по-бавна машина не.
   * Точно това прави клас Ж2 опасен: прозорецът е тесен и се отваря другаде.
   *
   * Затова се повтаря ЦЯЛАТА тройка, а условието е ПОСЛЕДИЦАТА — новият ред.
   * Успее ли първият опит, втори няма: редът се появява веднага.
   */
  await dokatoStane(
    p,
    async () => {
    ...
    () => p.$$eval('[data-etap="Акт 17"]', (e) => e.length > 0),
    'новият етап „Акт 17" се появява в таблицата',
  );
