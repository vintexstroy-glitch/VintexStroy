...
export function butonIstoriya(vid: string, id: string): string {
  // Знак отпред, дума до него (И101 т.2 · ADR-045). Името на класа остава —
  // проходът и CSS-ът го познават по него, а не по разметката вътре.
  //
  // ДОБАВЕНОТО `vtorichen malak` не е козметика: дотук този бутон беше ръчен
  // БЛИЗНАК на второстепенния — свой фон, своя рамка, своя височина — и точно
  // затова оставаше ИЗВЪН групата действия (ADR-057). Тоест редът носеше два
  // лоста един до друг, те не се побираха в колоната си и се пренасяха на нов
  // ред: реда на Имоти беше 67px, от които 59 бяха бутони. „История" е
  // второстепенно действие като всяко друго и влиза при своите.
  return butonSIkona({
    ikona: 'istoriya',
    tekst: 'История',
    title: 'История · кой, какво, кога',
    klas: 'vtorichen malak butoncheto-istoriya',
