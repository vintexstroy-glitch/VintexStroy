...
describe('пинът · трите извора се твърдят с ръка (резен 46 · група В)', () => {
  it('изворите на авто-дело са ТРИ, и то тези', () => {
    // Броят И имената. Само броят би пуснал преименуване; само имената — четвърти.
    expect(IZVORI_NA_AVTODELO).toHaveLength(3);
    expect([...IZVORI_NA_AVTODELO]).toEqual(['вноска', 'преписка', 'среща']);
  });
});
