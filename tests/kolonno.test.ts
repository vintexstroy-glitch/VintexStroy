...
describe('правило 23 има викащ в живия код', () => {
  ...
  it('и трите входа са пазени · двете полета през predPole, групата направо', () => {
    // Трите входа са: числовата клетка, клетката на добавка (резен 79) и
    // групата (Ctrl+D, която не отваря поле). Двете полета минават през ЕДИН
    // пазач — `predPole` — за да няма втори, който да се разхлаби отделно;
    // затова викащите му се броят, а голият пазач остава на две места:
    // вътре в него и в групата.
    const prezPredPole = IZVOR.match(/const vrata = predPole\(\)/g) ?? [];
    expect(prezPredPole.length).toBe(2);
