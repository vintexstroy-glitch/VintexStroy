...
describe('правило 23 има викащ в живия код', () => {
  ...
  it('и трите входа са пазени · единичният, групата И добавката', () => {
    // Ctrl+D не минава през отварянето на поле. Един пазач би оставил групата
    // отворена врата към заключената единична клетка. Третият вход е клетката
    // на ДОБАВЕНА колона (резен 79) — свой клон, същият пазач.
    const pazachi = IZVOR.match(/mozheDaPopraviKletka\(rolyata\)/g) ?? [];
    expect(pazachi.length).toBe(3);
  });
});
