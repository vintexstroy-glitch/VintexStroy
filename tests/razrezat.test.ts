...
describe('по СЕКТОР и по ПОТОК', () => {
  ...
  it('ПОТОКЪТ го носи само разходът · приходът пада в кофа с ИМЕ', async () => {
    ...
    // се слеят. Липсата се ВИЖДА — и оттук нататък това се ПРОВЕРЯВА, не се
    // казва: под коментара стоеше сравнение на изхода срещу същата константа,
    // тъй че изпразнена, тя щеше да мърда от двете страни и тестът да мълчи.
    expect(BEZ_STOYNOST).not.toBe('');
    const d = stend();
    await nasadi(d);
    const redove = obobshteniRedove(
      koloniNaTakta('mesets', '2026-08-15'),
