...
describe('групите · пет заглавия, нито едно празно', () => {
  ...
  /**
   * ПРАЗНА ГРУПА НЕ ИЗЛИЗА · заглавие без нищо под себе си не е ориентир, а
   * въпрос: „какво е трябвало да има тук и защо го няма?".
   *
   * ДОТУК служителят изобщо не виждаше „ХОРА И ПРАВА". От резен 112 (ADR-158)
   * екранът Служители е подтаб там и той вижда СВОЯ лист — но не и правата:
   * групата му е с ЕДНА тема, не с две. Разликата е точно между „работя с хора"
   * и „раздавам права" (И57).
   */
  it('празно заглавие НЕ се рисува · и служителят вижда САМО своята тема в „ХОРА И ПРАВА"', () => {
    for (const koy of KOY_GLEDA) {
      for (const g of temiPoGrupi(koy)) expect(g.temi.length, `${koy} · ${g.grupa.klyuch}`).toBeGreaterThan(0);
    }
    const negovite = temiPoGrupi('sluzhitel').find((g) => g.grupa.klyuch === 'hora');
    expect(negovite?.temi.map((t) => t.klyuch)).toEqual(['sluzhiteli']);
  });
...
describe('пиновете · броевете се твърдят с ръка (резен 46 · група В)', () => {
  it('групите са ПЕТ, а темите — ДЕВЕТНАЙСЕТ', () => {
    expect(GRUPI).toHaveLength(5);
    expect(TEMI).toHaveLength(19);
  });
});
