...
describe('сравнителят по вид (ADR-014)', () => {
  ...
  it('евро — по центове, с отрицателните най-отпред', () => {
...
describe('сметката на избора', () => {
  ...
  it('средното се закръгля до цент и НЕ влиза обратно в сбора', () => {
    const s = smetniIzbora([
      { tekst: 'а', st: 100 },
      { tekst: 'б', st: 100 },
      { tekst: 'в', st: 101 },
    ]);
    expect(s.sbor_st).toBe(301); // сборът е от точните центове
...
describe('редакторите на клетки', () => {
  it('наемът: чете и пише цели центове, изрича се в евро', () => {
...
describe('групирането по колона', () => {
  ...
  it('групата СУМИРА — само евро колоните, в цели центове', () => {
    const g = grupiraj(NAEMI, KOL[0]!, '2026-08-24');
