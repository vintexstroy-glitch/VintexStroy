...
describe('пиновете · валутите се твърдят с ръка (резен 46 · група В)', () => {
  it('валутите са ДВЕ и се пишат по СВОИТЕ норми', () => {
    // Правило 3: левът не съществува, курс няма, а изписването живее във
    // валутата. Затова тук се твърдят ЗНАКЪТ и СТРАНАТА му, не се четат от нея.
    expect(EVRO.znak).toBe('€');
    expect(DOLAR.znak).toBe('$');
    expect(EVRO.predi).toBe(false);
    expect(DOLAR.predi).toBe(true);
    // И ДРОБНИТЕ: „и двете със сто цента" (правило 3) се твърди, не се чете.
    expect(EVRO.drobni).toBe(100);
    expect(DOLAR.drobni).toBe(100);
  });
});
