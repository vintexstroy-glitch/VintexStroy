...
describe('видът на колоната се СМЯТА от данните', () => {
  ...
  it('ГОЛОТО цяло число е ЧИСЛО, не пари · телефонът не е сума', () => {
    // Намерено в неговия файл: „телефон" излизаше „евро", защото всяка клетка
    // се четеше като сума. Телефон в сбор на пари е повреда като текст, взет
    // за пари — само отзад напред.
    expect(vidNaKolonata(['0889805575', '0896252032'])).toBe('chislo');
    expect(vidNaKolonata(['147800', '56300'])).toBe('chislo');
  });

  it('а знакът, стотинките и интервалът за хиляди правят колоната ПАРИ', () => {
    expect(vidNaKolonata(['147800', '56 300'])).toBe('evro');
    expect(vidNaKolonata(['147800,00', '56300'])).toBe('evro');
    expect(vidNaKolonata(['1 234,56 €'])).toBe('evro');
  });
});
...
/**
 * РЕДЪТ НА ГЛАВАТА · функцията беше построена, но НЕПРОВЕРЕНА.
 *
 * Три нарочни счупвания минаха наведнъж („главата е винаги първи ред" · „и ред
 * без данни под него е глава" · „и числовият ред е глава"), защото единственият
 * ѝ свидетел беше проходът — а той води ЕДИН файл, в който трите условия
 * съвпадат. Функция, чиито условия не се проверяват поотделно, е три
 * непроверени функции.
 */
describe('на кой ред е главата', () => {
  it('заглавието в ЕДНА клетка не е глава · главата е под него', () => {
    expect(
      redatNaGlavata({
        ime: 'л',
        redove: [
  ...
  it('ред с ЧИСЛО не е глава · имената не са числа', () => {
    expect(
      redatNaGlavata({
        ime: 'л',
        redove: [
  ...
  // ТРИ реда, не два: при два последният никога не е кандидат (обходът спира
  // един преди края), тъй че условието „има ли данни отдолу" не се пита изобщо
  // и счупването му минаваше.
  it('ред БЕЗ данни под него не е глава · глава без редове не е глава', () => {
    expect(
      redatNaGlavata({
        ime: 'л',
        redove: [
  ...
  it('и когато главата е още на първия ред, отговорът е нула', () => {
    expect(
      redatNaGlavata({ ime: 'л', redove: [['апартамент', 'цена'], ['ателие А', '1000']] }),
    ).toBe(0);
  });

  it('таблица, в която НИЩО не прилича на глава, пада на първия ред · не отказва', () => {
    expect(redatNaGlavata({ ime: 'л', redove: [['1'], ['2']] })).toBe(0);
    expect(redatNaGlavata({ ime: 'л', redove: [] })).toBe(0);
  });
});
