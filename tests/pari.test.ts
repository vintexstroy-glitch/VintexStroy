/**
 * ПАРИТЕ · цели центове, никакъв float (П2.1).
 */
...
describe('цели центове', () => {
  it('33_33 + 66_67 = 100_00 точно', () => {
    expect(sabiri(tsentove(33_33), tsentove(66_67))).toBe(100_00);
  });

  it('отказва дробни центове', () => {
  ...
  it('изваждането е точно там, където float греши', () => {
    // 0.1 + 0.2 !== 0.3 при float; при центове няма такъв въпрос.
    expect(sabiri(tsentove(10), tsentove(20))).toBe(30);
    expect(izvadi(tsentove(30), tsentove(20))).toBe(10);
  });
});
...
describe('разпределяне без загуба на цент', () => {
  it('сборът на частите е точно изходната сума', () => {
    for (const [suma, chasti] of [
      ...
      const parcheta = razpredeli(tsentove(suma), chasti);
  ...
  it('остатъкът отива към първите части', () => {
    expect(razpredeli(tsentove(100_00), 3)).toEqual([3334, 3333, 3333]);
  });

  it('работи и с отрицателна сума (сторно на разсрочено)', () => {
    const parcheta = razpredeli(tsentove(-100_00), 3);
    expect(sabiri(...parcheta)).toBe(-100_00);
  });

  it('отказва безсмислен брой части', () => {
    expect(() => razpredeli(tsentove(100), 0)).toThrow(GreshkaPari);
    expect(() => razpredeli(tsentove(100), 2.5)).toThrow(GreshkaPari);
  });
});

describe('за четене от човек', () => {
  it('пише сумата с две цифри след запетаята', () => {
    expect(kakvoPishe(tsentove(100_00))).toBe('100,00\u202F€');
    expect(kakvoPishe(tsentove(5))).toBe('0,05\u202F€');
    expect(kakvoPishe(tsentove(-1234_56))).toBe('-1\u202F234,56\u202F€');
  });
});
...
describe('за екрана · числото от Огледалото е гол number', () => {
  it('пише същото, каквото и маркираният път', () => {
    expect(pishi(123_456)).toBe(kakvoPishe(tsentove(123_456)));
    expect(pishiVPole(123_456)).toBe(zaPisane(tsentove(123_456)));
  });

  it('НЕВЪЗМОЖНОТО ЧИСЛО СЕ КАЗВА, не минава за нула', () => {
    // Дотук екраните пишеха `kakvoPishe(x as never)` — а `as never` изключва
    // проверката ИЗЦЯЛО. Половин цент или NaN рисуваше „NaN €" на екран за
    // пари: тихо грешно число, което изглежда като число.
    expect(pishi(1.5)).toContain('не е цели центове');
    expect(pishi(Number.NaN)).toContain('не е цели центове');
    expect(pishi(undefined as unknown as number)).toContain('не е цели центове');
    expect(pishi(Number.POSITIVE_INFINITY)).toContain('не е цели центове');
  });
  ...
  it('нулата и отрицателното са НОРМАЛНИ числа, не грешки', () => {
    expect(pishi(0)).toBe(kakvoPishe(tsentove(0)));
    expect(pishi(-500_00)).toBe(kakvoPishe(tsentove(-500_00)));
