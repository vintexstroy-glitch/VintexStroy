...
export interface ModelNaTablitsa {
  ...
  /**
   * НА КОЙ ТАБ СТОИ ТОЗИ ХЕДЪР · ключът на екран от лентата · празно значи
   * „още не е сложен на таб" (И103).
   *
   * Негови думи: „хедърите на всички таблици с имена и подредени КАКТО СА ПО
   * ТАБОВЕТЕ В МЕНЮТО". Значи подредбата иска табът да е ИЗВЕСТЕН — а вносният
   * хедър идва от файл и не знае нищо за менюто.
   *
   * ЗАЩО СЕ ПИТА, А НЕ СЕ ГАДИ. „Ще познаем по данните" вече беше пробвано и се
   * счупи (ADR-014). Хедър, сложен на грешен таб по усет, дава грешна група при
   * раздаването на права — тиха грешка точно там, където се решава кой какво
   * вижда. По-добре празно и КАЗАНО, отколкото познато и сгрешено.
   *
   * ЗАЩО ПОЛЕ, А НЕ НОВО СЪБИТИЕ. Табът е свойство на хедъра, както номерът на
   * връзката и видът на стойността. Поправка = ново `МоделЗаписан` със същия
