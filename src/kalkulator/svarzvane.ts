...
/**
 * СЪЩИЯТ ключ, но за когото има нужда отвън · празно значи „името не носи номер".
 *
 * Изнесен при резен 107 (ADR-169): добавянето на активи от Журнала трябва да
 * познава дубликата ПО СМИСЪЛ, не по буква. „АП. № 1" от книгата и
 * „Апартамент 1" от файла са един и същ обект — де-дуп по дословен низ би ги
 * броил два пъти, точно каквото този модул съществува да предотврати.
 */
export function klyuchNiz(ime: string): string {
  const k = klyuchOtIme(ime);
  return k ? nizNaKlyuch(k) : '';
}
