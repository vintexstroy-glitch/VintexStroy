...
export const OPISANIE: Readonly<Record<Vazmozhnost, string>> = Object.freeze({
  ...
  'svarzhi-ii':
    ...
    'Самото свързване с доставчик на модел е добавка с цена и още не е започнало.',
});
 ...
 *
 * ИИ-то СТОИ ТУК, макар таблото му вече да е построено (И92 т.10 · ADR-026) —
 * и това не е пропуск. Построена е УПРАВНИЦИЯТА: картата, протоколът,
 * законите, задачите, входящата кутия и журналът. Онова, което възможността
 * ОБЕЩАВА с името си — самото СВЪРЗВАНЕ с доставчик на модел — е негови думи
 * „отделен проект, който не е започнал". Докато го няма, предложенията се
 * вписват от човека, а етикетът „скоро" казва честно докъде е стигнало.
 */
