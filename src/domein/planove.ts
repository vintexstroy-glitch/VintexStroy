 ...
 *    РАБОТЯТ ОФЛАЙН — ако си сваля приложение, с което работя с колеги като
 *    служител на фирмата, и нямам обхват, аз пак трябва да имам достъп да
 *    вкарвам, и когато имам — да се ъпдейтне в системата."
 *
...
export const OPISANIE: Readonly<Record<Vazmozhnost, string>> = Object.freeze({
  ...
  ogledala: 'Изгледи по обект и по контрагент — кой носи и кой дължи.',
  'drugi-imeyli':
  ...
  'roli-za-dostap': 'Роля на всеки добавен имейл: редактира или само наблюдава.',
  'kolonno-pravo':
    'Коя колона е скрита за кой служител. Скритата ПАК се смята — сборът ѝ ' +
    'остава. Редакцията идва от ролята и от вида на колоната, не оттук.',
 ...
 * „скоро", без бутон, който лъже.
 *
 * ДНЕС Е ПРАЗЕН, и това е новина, не пропуск. `svarzhi-ii` стоеше тук, докато
 * беше построена само УПРАВНИЦИЯТА — карта, протокол, закони, задачи, кутия,
 * журнал — а онова, което възможността ОБЕЩАВА с името си, го нямаше.
 * Свързването вече е построено (ADR-029: бутон, ключ, промпт от протокола,
 * потвърждение с имейл), затова етикетът „скоро" слезе: надпис „скоро" върху
 * работещ бутон е точно толкова лъжа, колкото бутон върху непостроено.
 *
 * Списъкът ОСТАВА, макар и празен. Следващата обявена, но непостроена
 * възможност ще влезе тук — а механизмът, който я показва честно, вече е
 * минат през проход и няма да се строи наново.
 */
export const OSHTE_NE_E_ZAPOCHNATO: ReadonlySet<Vazmozhnost> = new Set<Vazmozhnost>();
