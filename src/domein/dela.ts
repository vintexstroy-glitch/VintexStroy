/**
 * ДЕЛАТА · Имот → Обект → Дело, плоско, с филтри.
 *
 ...
 * **ТРИ КОЛОНИ С ФИЛТРИ**. Имот, Обект и Дело са колони на един ред — всяка
 * със своя номенклатура и свой филтър. „Не три нива" от същата дума е
 * НАДЖИВЯНО на 02.09 (И131 · ADR-153 §2 т.3): Обектът е ПОД Имота — има Имот
 * без Обект, няма Обект без Имот (пазачът е резен 99). Таблицата остава плоска,
 * колоните не се сгъват; „Място" в Журнала (`myasto`) е неговият Имот — ключ,
 * не дума (резен 96 · ADR-155).
 *
...
export interface Delo {
  ...
  readonly ime: string;
  /**
   * ЧАСЪТ · по избор: „Нека и делото да има право за вкарване на час, когато
   * е необходимо" (И124 т.1). Празно значи „само дата"; дните до срока пак се
   * броят по КАЛЕНДАР — часът не мени светофара (образецът на преписката).
   */
