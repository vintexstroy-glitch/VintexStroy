...
export interface Koefitsient {
  ...
  /**
   * МЕСЕЧЕН ПО ПРИРОДА · начисленото и ДДС-то са месечни понятия.
   *
   * Дотук: „появява се САМО при стъпка МЕСЕЦ" (11.08). От И135 (03.09) не
   * изчезва при друга стъпка, а се КАЗВА — виж `belezhkaZaStapkata`.
   */
...
/**
 * КОИ КОЕФИЦИЕНТИ ПРИ ТАЗИ СТЪПКА · ВСИЧКИТЕ (резен 116 · ADR-162).
 *
 * Негово, 03.09 (И135): „Махаш мои коефициенти, махни го глупаво е." Дотук
 * месечните по природа (събираемост · ДДС към приход) ИЗЧЕЗВАХА при стъпка,
 * различна от месец — по думата му от 11.08 („когато са под година
 * коефициентите не се виждат"). Последната дума бие (правило 28): стъпката и
 * периодът менят СТОЙНОСТИТЕ, не списъка. Месечната природа не се крие, а се
 * КАЗВА до числото (`belezhkaZaStapkata`) — правило 15.
 *
 * Подписът пази стъпката нарочно: викащият казва при какво пита, и ако утре
 * пак се появи причина за стеснение, тя ще мине оттук с ADR, не с `if`.
 */
export function zaStapka(_stapka: 'mesets' | 'drug'): readonly Koefitsient[] {
  return KOEFITSIENTI;
}

/** Какво се КАЗВА до месечния по природа, когато стъпката не е месец · празно иначе. */
export function belezhkaZaStapkata(k: Pick<Koefitsient, 'samoMesechen'>, stapka: string): string {
  return k.samoMesechen && stapka !== 'mesets'
    ? 'месечен по природа · тук е сметнат за цялото парче от периода'
    : '';
}
 ...
 *
 * „Само показани суми за избрания период от време И СТЪПКАТА." Стъпката е
 * решение за РЯЗАНЕТО; кои коефициенти се показват вече не зависи от нея
 * (И135, по-горе).
 */
/**
