...
/**
 * НА КОЙ ТАБ СТОИ ХЕДЪРЪТ · И103 · и защо се ПИТА, а не се гади.
 *
 * Негови думи: „хедърите на всички таблици с имена и подредени КАКТО СА ПО
 * ТАБОВЕТЕ В МЕНЮТО". Подредбата иска табът да е известен; вносният хедър идва
 * от файл и не знае нищо за менюто. Питаме веднъж — тук.
 *
 * ЖИВИТЕ ЕКРАНИ СЕ ПОДАВАТ, не се преписват. Регистърът им е ЕДИН и е
 * `EKRANI` (`app/ekranite.ts`); втори списък тук би се разминал при първия нов
 * екран — точно правило 17. Затова функцията ПРИЕМА кои са живите и отказва
 * ключ, който не е между тях.
 *
 * ПРАЗНОТО МАХА ЗАПИСА, както при номера на връзка: „още не е сложен на таб" е
 * състояние, не липса — то се БРОИ и се показва (правило 15).
 */
export function dayEkran(
  m: ModelNaTablitsa,
  ekran: string,
  zhiviEkrani: readonly string[],
  rolya: RolyaNaChovek,
): ModelNaTablitsa {
  samoUpravitel(rolya, 'Табът на хедъра');
  const chist = ekran.trim();
  if (chist === '') {
    const bez = { ...m };
    delete bez.ekran;
    return Object.freeze(bez);
  }
  if (!zhiviEkrani.includes(chist)) {
    throw new GreshkaRedaktor(
      `„${chist}" не е екран в лентата. Табът се избира от менюто, не се пише.`,
    );
  }
  return Object.freeze({ ...m, ekran: chist });
}
